...
The project starts with YOLO-style detectors. YOLO divides the image into many candidate regions and predicts box position and class in one forward pass. It is fast, which makes it useful for later ONNX and TensorRT deployment comparisons.
...
A classification model usually has a feature extractor and a classification head. The feature extractor turns the image into a vector. The head turns the vector into class probabilities. The project starts with ResNet18 because it is easy to inspect, stable to train, and useful as a baseline against lighter models.
...
The project plans to use PatchCore. PatchCore stores local features from normal images. During inference, it compares local features from a test image with the normal feature memory. If a local feature is far from the normal memory, that region is more likely to be abnormal.
...
EL images often have uneven brightness, silicon texture interference, busbar interference, and low-contrast microcracks. The project will test CLAHE, gamma correction, and simple filtering.
